...
void Lab5::Update(float deltaTimeSeconds)
{
...

    {
        ...
        modelMatrix = glm::rotate(modelMatrix, RADIANS(80.0f), glm::vec3(0, 0, 1));
        RenderMesh(meshes["box"], shaders["VertexNormal"], modelMatrix);
    }

    {
        ...
        modelMatrix = glm::rotate(modelMatrix, RADIANS(100.0f), glm::vec3(0, 0, 1));
        RenderMesh(meshes["box"], shaders["VertexNormal"], modelMatrix);
    }
...
void Lab5::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->KeyHold(GLFW_KEY_1)) {
            angularStep = angularStep + deltaTime * cameraSpeed;
            if (mode == "perspective") {
                projectionMatrix = glm::perspective(RADIANS(angularStep), window->props.aspectRatio, zNear, zFar);
            }
...
void Lab5::OnMouseMove(int mouseX, int mouseY, int deltaX, int deltaY)
{
    ...
    if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->GetSpecialKeyState() == 0) {
            ...
            camera->RotateFirstPerson_OX(atan2(deltaY + sensivityOY, deltaX + sensivityOX));
            camera->RotateFirstPerson_OY(atan2(deltaY + sensivityOY, deltaX + sensivityOX));
        }
        ...
        if (window->GetSpecialKeyState() & GLFW_MOD_CONTROL) {
            ...
            // OX and OY using `deltaX` and `deltaY`. Use the sensitivity
            // variables for setting up the rotation speed.
            camera->RotateThirdPerson_OX(atan2(deltaY + sensivityOY, deltaX + sensivityOX));
            camera->RotateThirdPerson_OY(atan2(deltaY + sensivityOY, deltaX + sensivityOX));
        }
    }
}
